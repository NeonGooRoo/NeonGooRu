...
var markdownItContainer__default = /*#__PURE__*/_interopDefaultLegacy(markdownItContainer);
...
var transformPreview = function (md, token, env) {
    ...
    // 组件绝对路径
    var componentPath = path.resolve(path.dirname(env.path), componentProps.path || '.');
    // 组件名
    ...
    // 组件源码
    var componentSourceCode = fs.readFileSync(componentPath, {
        encoding: 'utf-8'
    });
    // 源码代码块（经过处理）
...
var containerDirectiveMount = function (md) {
    md.use(markdownItContainer__default["default"], 'preview', {
        marker: ':',
...
var parseContainerTag = function (md) {
    ...
    md.renderer.rules.container_preview_open = function (tokens, idx, options, env, self) {
        ...
        // 组件绝对路径
        var componentPath = path.resolve(path.dirname(env.path), componentRelativePath || '.');
        // 后缀名
        var suffixName = componentPath.substring(componentPath.lastIndexOf('.') + 1);
        // 组件源码
        var componentSourceCode = fs.readFileSync(componentPath, {
...
exports.containerPreview = containerPreview;
